...
st.markdown("""
<style>
:root { --prosaga-blue:#2563eb; --prosaga-dark:#0f172a; }
.block-container { padding-top: 1.6rem; }
h1, h2, h3 { color: var(--prosaga-dark); }
[data-testid="stSidebar"] { background-color: #f5f7fb; border-right: 1px solid #e2e8f0; }
[data-testid="stSidebar"] h1 { color: var(--prosaga-blue); font-weight: 900; letter-spacing: 1px; }
.stButton>button[kind="primary"] { background-color: var(--prosaga-blue); border: none; }
...
def page_run_detail():
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "Kategori & Strategi", "Preprocessing", "Log Iterasi", "Silhouette per Produk", "Visualisasi"
    ])
    ...
    with tab2:
        st.subheader("Log Tahapan Preprocessing")
        for p in prep:
            st.write(f"{p['step_order']}. {p['message']}")

        st.subheader("Normalisasi Min-Max per Baris")
        st.caption("Rumus: x' = (x - min(x)) / (max(x) - min(x)). Nilai asli vs hasil normalisasi (0-1).")
        df_norm = pd.DataFrame(rows)
        if not df_norm.empty:
            show_n = st.slider("Jumlah baris ditampilkan", 5, min(200, len(df_norm)), min(20, len(df_norm)))
            table_norm = df_norm[["product", "quantity", "quantity_norm", "frequency", "frequency_norm",
                                   "total_sales", "total_sales_norm"]].head(show_n)
            table_norm.columns = ["Produk", "Quantity Sold", "Quantity Sold (norm)", "Transaction Frequency",
                                   "Transaction Frequency (norm)", "Total Sales", "Total Sales (norm)"]
            st.dataframe(table_norm, use_container_width=True, hide_index=True)
            csv = df_norm.to_csv(index=False).encode("utf-8")
            st.download_button("Unduh seluruh tabel normalisasi (CSV)", csv, file_name=f"normalisasi_run{run['id']}.csv")
    ...
    with tab3:
        for algo in ["K-Means", "Fuzzy C-Means"]:
            ...
            for l in algo_logs:
                ...
                metric_name = "SSE" if "inertia" in detail else "Objective"
                metric_val = detail.get("inertia", detail.get("objective"))
                shift = detail.get("shift", detail.get("max_change"))
                summary_rows.append({"Iterasi": l["iteration"], metric_name: round(metric_val, 6),
                                      "Perubahan (shift/max_change)": round(shift, 8) if shift is not None else None})
            st.dataframe(pd.DataFrame(summary_rows), use_container_width=True, hide_index=True)

            last = json.loads(algo_logs[-1]["details_json"])
            centers = last.get("centers_after", last.get("centers"))
            if centers is not None:
                st.markdown(f"**Centroid akhir ({algo}), setelah dikanonikalisasi Laris/Sedang/Kurang Laris:**")
                centers_df = pd.DataFrame(centers, columns=FEATURES)
                centers_df.index = ["Cluster 1 (Laris)", "Cluster 2 (Sedang)", "Cluster 3 (Kurang Laris)"][:len(centers_df)]
                st.dataframe(centers_df.round(6), use_container_width=True)

    with tab4:
        st.subheader("Perhitungan Silhouette Score per Produk")
        st.caption("s(i) = (b(i) - a(i)) / max(a(i), b(i))")
        df_sil = pd.DataFrame(rows)
        if not df_sil.empty:
            col_sil = "silhouette_kmeans" if run["best_algorithm"] == "K-Means" else "silhouette_fcm"
            table_sil = df_sil[["product", "category", col_sil]].rename(
                columns={"product": "Produk", "category": "Kategori", col_sil: "Silhouette s(i)"}
            )
            st.dataframe(table_sil, use_container_width=True, hide_index=True)
...
def page_backup():
    ...
    if st.button("Download Backup Database Sekarang"):
        ...
        t0 = time.time()
        with st.spinner("Menyalin database..."):
...
def page_admin():
    st.header("Admin — Kelola Akun & Audit Log")
    tab1, tab2, tab3 = st.tabs(["Kelola Staff", "Buat Akun Baru", "Audit Log"])
    ...
    with tab1:
        ...
        for u in users:
            ...
            c2.write("Aktif" if u["is_active"] else "Nonaktif")
            actor = st.session_state["user"]["username"]
            if u["username"] != actor:
                if u["is_active"]:
                    if c3.button("Nonaktifkan", key=f"deact_{u['username']}"):
                        db.set_user_active(u["username"], False, actor)
                        st.rerun()
                else:
                    if c3.button("Aktifkan", key=f"act_{u['username']}"):
                        db.set_user_active(u["username"], True, actor)
                        st.rerun()
            with c4.popover("Ganti Password"):
                new_pw = st.text_input("Password baru", type="password", key=f"pw_{u['username']}")
                if st.button("Simpan", key=f"savepw_{u['username']}"):
                    ok, msg = db.admin_change_password(u["username"], new_pw, actor)
                    if ok:
    ...
    with tab2:
        ...
        if submitted:
            ok, msg = db.create_user(u, p, role=role, actor=st.session_state["user"]["username"])
            if ok:
    ...
    with tab3:
        ...
        if logs:
            df_log = pd.DataFrame(logs)[["created_at", "username", "action", "description"]]
            df_log.columns = ["Waktu", "Username", "Aksi", "Keterangan"]
...
def main():
    ...
    st.sidebar.caption("SALES CLUSTERING • K-MEANS + FCM")
    st.sidebar.write(f"Masuk sebagai **{user['username']}** ({user['role']})")
    menu_items = ["Analisis Baru", "Dashboard", "Detail Analisis", "Perbandingan Analisis", "Backup & Restore"]
    if user["role"] == "admin":
